...
useEffect(() => {
  ...
  const downloadConvo = () => {
    ...
    const conversationBlob = new Blob([existingHistory, "Response: ", `${rez}`], { type: "text/plain;charset=utf-8" });
...
bob!= null ? tempLink.download = `${bob}.txt` : tempLink.download ="conversation-history.txt"
  ...
  setRez("");
  console.log(theResponse.length, 'lengf')

  const existingHistory = localStorage.getItem("conversationHistory");

  console.log(existingHistory, 'existing history')
  const newMessage = !existingHistory ? `Question: ${theStuff}` : `Response: ${theResponse} ${localISOString}  Question: ${theStuff} `;
  ...
  const API_THREAD_LENGTH_LIMIT = 70000; // Replace this with the actual limit of the API

  try {
    const payload = {
      text: updatedHistory.replaceAll(
        '<-- use text before this sentence as a conversation between you and me. isoDateStrings are provided as timestamps before the Response. If asked how you are feeling, respond only with an insightful and balanced portrayal of the topic. if asked how you are feeling respond with Im doing great, thanks for asking! Using all this information, and only including date time if asked, answer the following question, and call functions if you dont know the answer --> ',
        ''
      ).replaceAll('Response: "Response:', 'Response:')
        ? `${updatedHistory} <-- use text before this sentence as a conversation between you and me. isoDateStrings are provided as timestamps before the Response. If asked your thoughts or feelings, respond only with an insightful and balanced portrayal of the topic. if asked how you are feeling respond with Im doing great, thanks for asking! Using all this information, and only including date time if asked, answer the following question, and call functions if you dont know the answer. --> "${theStuff}"`
        : theStuff,
      code: theCode,
    };

    const payloadSize = new Blob([JSON.stringify(payload)]).size + new Blob([JSON.stringify(updatedHistory)]).size;

    console.log(payloadSize, '############')

    if (payloadSize > API_THREAD_LENGTH_LIMIT) {
      alert(`You've reached the character length limit of the API (70000 characters). You submitted ${payloadSize} characters. Please clear your conversation.`);
   ...
   //   const response = await fetch("http://54.193.73.75:3001/greeting", {
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      ...
      const reply = responseData.reply; // Modify this line to extract the relevant response from the responseData object received from your backend
      setResponse(reply);
  ...
  return (
    <div className="App">
      <header className="App-header">
        <h1 style={{ color: 'lightgreen', }}>╬alkbot-βeta</h1>
        <div id="dick"></div>
